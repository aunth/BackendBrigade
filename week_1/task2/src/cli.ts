...
async function viewPendingRequests() {
  ...
  holidayRequests.filter(request => request.status === 'pending').forEach(request => {
    console.log(`Request ID: ${request.idForRequest}, Employee ID: ${request.employeeId} (${getNameById(request.employeeId)}), Start Date: ${request.startDate.toLocaleDateString('en-CA')}, End Date: ${request.endDate.toLocaleDateString('en-CA')}, Status: ${request.status}`);
  });
}
