...
export const RequestSchema = new Schema({
  ...
  }, {collection: 'Requests', id: false, autoIndex: true });
...
}, {collection: 'Employees', id: false, autoIndex: true });

export const DepartmentSchema = new Schema({
	_id: { type: Types.ObjectId, required: true },
    name: { type: String, required: true },
    maxConsecutiveDays: { type: Number, required: true },
	blackoutPeriods: { type: [Types.ObjectId], required: true }
}, {collection: 'Departments', id: false, autoIndex: true });

const holidaySchema = new Schema({
	_id: {type: Types.ObjectId, required: true},
	startDate: { type: Date, required: true },
	endDate: { type: Date, required: true },
  }, {collection: 'Holidays', id: false, autoIndex: true });
